...
function uniqueArr(arr) {
    return Array.from(new Set(arr.map(function(ele) {
...
var WhiteList = /** @class */ (function() {
    ...
    WhiteList.prototype.fuzzyMatch = function(list, size) {
        return list.filter(function(s) {
            return isNumeric(s.width) ? s.width >= size.width : s.height >= size.height;
        });
    };
    /**
     * 精确匹配策略
...
function setSize(url, width, height, whiteList, opts) {
    // 按尺寸白名单匹配宽高
    var matchedSize = whiteList.getMatchedSize({
        width: width,
        height: height
    }, opts.fuzzyMatch);
...
function setDPRCompatibleOpts(opts) {
    ...
    if (opts.a) { /** 高级裁剪 */
        if (!/^\d+-\d+-\d+-\d+$/.test(opts.a)) {
            ...
            opts.a = prefix.map(function(p) {
                return Math.floor(Number(p) * zoom);
            }).join('-');
        }
    }
    if (opts.rc) { /** 区域裁剪 */
