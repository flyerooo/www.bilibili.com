  ...
  <div>
    <van-popover
      ...
      <div class="vp-container">
        ...
        <p class="nickname"
          :style="{
            'padding-top': (userInfo.pendant && userInfo.pendant.image) ? '52px' : '42px',
            'color': userInfo.vip ? userInfo.vip.nickname_color : '#212121'}"
        ...
        <a v-if="isVip"
          :style="labelColor"
          class="vip"
          v-text="userInfo.vip && userInfo.vip.label ? userInfo.vip.label.text : ''"
...
export default {
  ...
  mounted() {
    ...
    if (this.iconType === 'custom') {
      bigParams.customIcon = this.userInfo.vip.avatar_subscript_url
    } else {
      bigParams.iconType = this.iconType
    }
  ...
  computed: {
    iconType() {
      if (this.verify === 'organization') {
        ...
        return 'personal'
      } else if (this.isVip && this.userInfo.vip && this.userInfo.vip.avatar_subscript_url) {
        return 'custom'
