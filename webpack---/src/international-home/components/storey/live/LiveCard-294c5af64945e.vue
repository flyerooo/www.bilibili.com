...
<script>
import { trimHttp, formatNum, customReport, allCustomReport } from 'g-public/js/utils'
...
export default {
  ...
  methods: {
    handleShow() {
      let marker_id = 0
      let marker_content = ''
      if (this.info.pendant_Info && Object.keys(this.info.pendant_Info).length) {
        marker_id = Object.keys(this.info.pendant_Info)[0]
        marker_content = this.info.pendant_Info[marker_id]
      }
      // 上报到数据平台
      customReport('live_card_show', {
        room_id: this.info.roomid,
        up_id: this.info.uid,
        parent_area_id: this.info.area_v2_parent_id,
        area_id: this.info.area_v2_id,
        live_status: 1,
